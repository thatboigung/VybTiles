...
export const BottomPlayer: React.FC<BottomPlayerProps> = ({
    ...
    return (
        <div className="h-24 bg-[#181818] border-t border-[#282828] px-4 flex items-center justify-between z-[100] animate-in slide-in-from-bottom duration-500">
            {/* Left: Song Info */}
            <div className="flex items-center gap-4 w-[30%] min-w-0">
                {currentSong.coverArt ? (
                    <img src={currentSong.coverArt} alt="Cover" className="w-14 h-14 rounded shadow-lg object-cover" />
                ) : (
            ...
            <div className="flex flex-col items-center gap-2 max-w-[40%] w-full">
                ...
                <div className="w-full flex items-center gap-2 group">
                    <span className="text-xs text-[#b3b3b3] tabular-nums font-mono">{formatTime(currentTime)}</span>
                    <div className="flex-1 h-1 bg-[#4d4d4d] rounded-full relative cursor-pointer group-hover:h-1.5 transition-all">
                        ...
                        {/* Slider thumb - hidden by default, visible on hover */}
                        <div
                    ...
                    <span className="text-xs text-[#b3b3b3] tabular-nums font-mono">{formatTime(duration)}</span>
                </div>
            </div>
